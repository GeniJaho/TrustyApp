...
const Signup = () => {
  ...
  const craftsmenSignup = () => {
    return(
      ...
      <form onSubmit={handleSubmit(craftsmenRegister) }>
        <input type="text" defaultValue='' placeholder="Full Name" {...register('full_name')}/>
        <p style={{fontSize: '14px', color: 'red', textAlign: 'center'}}>{errors.full_name?.message}</p>

        <input type="text" defaultValue='' placeholder="Username" {...register('username')}/>
        <p style={{fontSize: '14px', color: 'red', textAlign: 'center'}}>{errors.username?.message}</p>

        <input type="text" defaultValue='' placeholder="Craft" {...register('craft')}/>
        <p style={{fontSize: '14px', color: 'red', textAlign: 'center'}}>{errors.craft?.message}</p>

        <input type="text" defaultValue='' placeholder="Price" {...register('price')}/>
        <p style={{fontSize: '14px', color: 'red', textAlign: 'center'}}>{errors.price?.message}</p>

        <input type="text" defaultValue='' placeholder="Language" {...register('language')}/>
        <p style={{fontSize: '14px', color: 'red', textAlign: 'center'}}>{errors.language?.message}</p>

        <input type="text" defaultValue=''  placeholder="Address" {...register('address')}/>
        <p style={{fontSize: '14px', color: 'red', textAlign: 'center'}}>{errors.address?.message}</p>

        <input type="password" defaultValue='' placeholder="Password" {...register('password')}/>
        <p style={{fontSize: '14px', color: 'red', textAlign: 'center'}}>{errors.password?.message}</p>

        <input type="password" defaultValue='' placeholder="Confirm Password" {...register('conf_password')}/>
        <p style={{fontSize: '14px', color: 'red', textAlign: 'center'}}>{errors.conf_password && "Password not matching!"}</p>
  ...
  return (
    <div className="signin">
      {toggleSignup ? craftsmenSignup() : userSignup()}
      <div className="bottom-signup">
        ...
        {toggleSignup ? <button onClick={()=> setToggleSignup(!toggleSignup)}>Sign Up as a Customer</button> : <button onClick={()=> setToggleSignup(!toggleSignup)}>Sign Up as a Craftsmen</button>}
      </div>
    </div>
  );
};
